-- Guild Bank Viewer -- bundled item name database
-- Sources (both MIT License, by Eric Mauser / Shagu):
--   pfQuest      https://github.com/shagu/pfQuest          (base Classic item catalog)
--   pfQuest-turtle https://github.com/KameleonUK/pfQuest-turtle (Turtle WoW-specific items)
-- Used only as a fallback search seed for the New Listing/New Bounty item search.
-- Items you actually see in-game are always learned live and take priority over this list.
...
-- Merge both into one table (turtle-specific entries win on ID collisions,
-- since they reflect this server's actual item data more precisely).
for id, name in pairs(GBV_ItemDB_base) do GBV_ItemDB[id] = name end
for id, name in pairs(GBV_ItemDB_turtle) do GBV_ItemDB[id] = name end
GBV_ItemDB_base = nil
GBV_ItemDB_turtle = nil
